...
#define CR_DEBUG 0
    ...
    public:
        ...
        inline T*   get()       const noexcept { return expired() ? nullptr : m_ptr; }
...
//        inline shared_ptr<T> lock() const;
        ...
        {
            weak_ptr(other).swap(*this); // copy swap
            return *this;
        }
            
        weak_ptr& operator=(std::nullptr_t) noexcept
        {
            weak_ptr().swap(*this); // copy swap
            return *this;
        }
    ...
    public:
        ...
        template <typename U> //typename = valid_cast<U>>
        shared_ptr(shared_ptr<U>&& ptr, T* p) noexcept
...
//        // @brief Copy construct from convertible types
//        template<typename U, typename = valid_cast<U>>
...
//            return shared_ptr<T>(new T(std::forward<Args>(args)...)); // no extra handling
    ...
    template<typename T, typename U, typename = array_consistency_check<T, U>>
    shared_ptr<T> static_pointer_cast(shared_ptr<U>&& ptr) noexcept
    {
        return shared_ptr<T>(std::move(ptr), static_cast<T*>(ptr.get()));
    }
    
    // dynamic cast of shared_ptr
    template<typename T, typename U, typename = array_consistency_check<T, U>>
    shared_ptr<T> dynamic_pointer_cast(shared_ptr<U>&& ptr) noexcept
    {
        T* p = dynamic_cast<T*>(ptr.get());
        return p ? shared_ptr<T>(std::move(ptr), p) : shared_ptr<T>();
    }

    template<typename T, typename U, typename = array_consistency_check<T, U>>
    shared_ptr<T> const_pointer_cast(shared_ptr<U>&& ptr) noexcept
    {
        typedef typename std::remove_extent<T>::type RT;
        return shared_ptr<T>(std::move(ptr), const_cast<RT*>(ptr.get()));
    }
...
//    }
    ...
    struct owner_less
    {
        ...
        bool operator()( cr::shared_ptr<T> const& x, cr::shared_ptr<U> const& y) const noexcept { return x.owner_before(y); }

        template <class T, class U>
        bool operator()( cr::shared_ptr<T> const& x, cr::weak_ptr<U> const& y) const noexcept { return x.owner_before(y); }

        template <class T, class U>
        bool operator()( cr::weak_ptr<T> const& x, cr::shared_ptr<U> const& y) const noexcept { return x.owner_before(y); }

        template <class T, class U>
        bool operator()( cr::weak_ptr<T> const& x, cr::weak_ptr<U> const& y) const noexcept { return x.owner_before(y); }

        typedef void is_transparent;
    };
    
    template <typename T>
    struct owner_less<cr::shared_ptr<T>>// : std::binary_function<cr::shared_ptr<T>, cr::shared_ptr<T>, bool>
    {
        typedef bool result_type;
        bool operator()(cr::shared_ptr<T> const& x, cr::shared_ptr<T> const& y) const noexcept { return x.owner_before(y); }
        bool operator()(cr::shared_ptr<T> const& x,   cr::weak_ptr<T> const& y) const noexcept { return x.owner_before(y); }
        bool operator()(cr::weak_ptr<T> const& x,   cr::shared_ptr<T> const& y) const noexcept { return x.owner_before(y); }
    };
    
    template <typename T>
    struct owner_less<cr::weak_ptr<T>>// : std::binary_function<cr::weak_ptr<T>, cr::weak_ptr<T>, bool>
    {
        typedef bool result_type;

        bool operator()(cr::weak_ptr<T> const& x,   cr::weak_ptr<T> const& y) const noexcept { return x.owner_before(y); }
        bool operator()(cr::shared_ptr<T> const& x, cr::weak_ptr<T> const& y) const noexcept { return x.owner_before(y); }
        bool operator()(cr::weak_ptr<T> const& x, cr::shared_ptr<T> const& y) const noexcept { return x.owner_before(y); }
    };

    template<typename T>
    inline void swap(weak_ptr<T>& x, weak_ptr<T>& y) noexcept { x.swap(y); }
    ...
    {
        return weak_ptr<T>(ptr, static_cast<T*>(ptr.get()));
    }
    
    template<class T, class U, typename = array_consistency_check<T, U>>
    weak_ptr<T> dynamic_pointer_cast(const weak_ptr<U>& ptr) noexcept
    {
        T* raw = dynamic_cast<T*>(ptr.get());
    ...
    {
        typedef typename std::remove_extent<T>::type RT;
        return weak_ptr<T>(ptr, const_cast<RT*>(ptr.get()));
    }
...
//        shared_ptr<T const> shared_from_this() const noexcept { return shared_ptr<const T>(weak_this); }
    ...
    void shared_ptr<T>::enable_weak_this(const enable_shared_from_this<U>* ptr, R* raw) noexcept
    {
        ...
        {
            ptr->weak_this = weak_ptr<RU>(weak_ptr<T>(*this), const_cast<RU*>(static_cast<const U*>(raw)));
        }
    }
}
...
namespace std
{
    ...
    template <typename T>
    struct hash<cr::shared_ptr<T>>
    {
        typedef cr::shared_ptr<T>      argument_type;
    ...
    // overload swaps
    template<typename T>
    inline void swap(cr::shared_ptr<T>& x, cr::shared_ptr<T>& y) noexcept { x.swap(y); }
    template<typename T>
    inline void swap(cr::weak_ptr<T>& x, cr::weak_ptr<T>& y) noexcept { x.swap(y); }
}
