...
bool UdpClient::Start(HandleMsgFunc func) {
    ...
    RakNet::ConnectionAttemptResult ret = m_rakPeer->Connect(ip, atoi(serverPort), password, passwordLength);
...
bool UdpClient::SendMsg(const NetPack& msg)
{
    return m_rakPeer->Send((const char*)msg.Buffer(), msg.Size(), LOW_PRIORITY, RELIABLE_ORDERED, 0, m_serverAddr, false) > 0;
}
